...
async def private_receive_handler(c: Client, m: Message):
    ...
    try:
        ...
        stream_link = f"http://videoit.ml/?api=1&PIN={Var.URL}{str(log_msg.id)}/{quote_plus(get_name(log_msg))}?hash={get_hash(log_msg)}"
        ...
        await m.reply_text(
            ...
            quote=True,
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("STREAM 🖥 & Dᴏᴡɴʟᴏᴀᴅ📥", url=stream_link) #Stream Link
                                               ]]) #Download Link
        )
